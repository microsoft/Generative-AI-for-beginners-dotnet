...
- **Demo A — Foundry chat (one live demo):** `samples/CoreSamples/BasicChat-05AIFoundryModels`
  ...
  - Highlight `IChatClient` and the one-line `client.GetResponseAsync(...)` call.
  - The sample asks a hardcoded question: `what is your model name?`
...
- **Demo B — swap the model (Foundry):** change **only** `AzureOpenAI:Deployment` to switch
  **`gpt-5.5` → `grok-4`**. Same code, same endpoint, same `IChatClient`.
  - Rerun the app with the **same hardcoded question** so the audience sees the model identity change.
...
- **Optional 20s:** mention streaming + structured output exist too (lesson 02), but keep this demo intentionally simple.
